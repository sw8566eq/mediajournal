...
export function EntryDetail({ mediaType, entry, onEdit, onDelete, onBack }: Props) {
  ...
  return (
    <div className="entry-detail">
      ...
      <div className={cover ? 'entry-detail-layout has-cover' : 'entry-detail-layout'}>
        <div className="entry-detail-meta-pane">
          <div className="meta-grid">
            {TYPE_FIELDS[mediaType].map((f) => (
              <div key={f.key}>
                <strong>{f.label}:</strong>{' '}
                {entry[f.key] === null || entry[f.key] === undefined ? '—' : String(entry[f.key])}
              </div>
            ))}
            <div>
        ...
        {cover && (
          <div className="entry-detail-cover-pane">
            <img src={cover} alt="" />
          </div>
        )}
      </div>
